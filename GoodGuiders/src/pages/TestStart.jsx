...
export const TestStart = () => {
    ...
    const fetchQuestions = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/questions/fetch", {
          ...
          body: JSON.stringify({
            class: state.class,
            subject: state.subjects, // send as array (your backend supports array now)
            type: state.testType,
  ...
  if (loading) return <div style={{ marginTop: "120px" }}>Loading questions...</div>;

  return (
    <div style={{ marginTop: "120px", padding: "0 100px" }}>
      <h3 style={{ marginBottom: "30px", fontSize: "35px" }}>Test Begins</h3>
      {questions.map((q, index) => (
        <div key={q._id || index} style={{ marginBottom: "25px" }}>
